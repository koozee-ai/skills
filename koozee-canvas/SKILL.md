...
已有项目时，每条会改画布的命令紧挨前面先 `project +get --project-id <id>`。`--place`、`--from`、`+link`、`--node-id`、`+upload --surface workflow`、`skill +run --write`、会写回的 `+await` 都要。节点和入线只用这一次的结果，不用上一轮信封，也不用更早的 `+get`。对人讲 `name`，边讲 `fromName` → `toName`。不要念 id。还没有项目 id 时不用先 get。
...
3. `--from`、`--to`、`--node-id` 只用刚那次 `project +get` 里还在的卡。不在图上的不要连、不要生成。
